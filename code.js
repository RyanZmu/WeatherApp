...
class UserWeather {
  ...
  buildUser () {
    const userLat = this.lat
    const userLong = this.long

    const weatherFetch = fetch(`https://api.weather.gov/points/${userLat},${userLong}`)
  ...
  displayWeather (forecastData) {
    ...
    if (forecastDisplayShort[0].includes('Cloudy')) {
      backgroundImage.style.backgroundImage = `url(${images[3]})`
    }
    // "url(" + images[x] + ")";
    // })
  }
